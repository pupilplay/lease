...
public class RoomController {
    ...
    public Result<RoomDetailVo> getDetailById(@RequestParam Long id) {
        return Result.ok(service.geteDetailById(id));
    }
    ...
    public Result updateReleaseStatusById(Long id, ReleaseStatus status) {
        service.update(new LambdaUpdateWrapper<RoomInfo>().eq(RoomInfo::getId, id).set(RoomInfo::getIsRelease,status));
        return Result.ok();
    }
    ...
    public Result<List<RoomInfo>> listBasicByApartmentId(Long id) {
        return Result.ok(service.list(new LambdaQueryWrapper<RoomInfo>().eq(RoomInfo::getApartmentId, id)));
    }
}
